...
export async function getCmsPermissionsData(): Promise<PermissionsGovernanceData> {
  ...
  const [roleRows, permissionRows, taskRows, assignmentRows, userRows] = await Promise.all([
    ...
    sql`SELECT id,module,view,_task,description,published,ordering,list_function FROM cic_permission_tasks WHERE published=true ORDER BY ordering,id`,
  ...
  return {
    roles,
    tasks: taskRows.map((task) => ({ id: String(task.id), module: text(task.module), view: text(task.view), task: text(task._task), description: text(task.description), published: Boolean(task.published), ordering: Number(task.ordering ?? 0), listFunction: text(task.list_function) })),
