...
export async function uploadMediaAction(formData:FormData) {
  ...
  const locale=formData.get('locale')==='en'?'en':'vi'; const {path,mime}=await uploadFile(file,locale);
  try{const input=mediaUploadRegistrationSchema.parse({storagePath:path,filename:file.name,mimeType:mime,fileSizeBytes:file.size,mediaType:mediaType(mime),locale,title:String(formData.get('title')||file.name),altText:String(formData.get('altText')||''),folderId:formData.get('folderId')||null}); const result=await createMediaAsset(input,actor);refresh();return result;}
