...
export async function searchPublishedContent(query: string): Promise<SearchResult[]> {
  ...
  if (!keyword) return [];
  const [products, projects, services, news, events] = await Promise.all([listPublishedProducts(), listPublishedProjects(), listPublishedServices(), listPublishedNews({ query, pageSize: 30 }), listPublishedEvents()]);
  const results: SearchResult[] = [
    ...
    ...news.items.map((item) => ({ id: item.id, type: 'news' as const, title: item.title, summary: item.summary, href: `/news/${item.slug}` })),
