...
export function CmsTrashConfirmDialog({
  ...
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center overflow-y-auto bg-slate-950/60 p-3 backdrop-blur-xs" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
      <div ref={dialogRef} role="alertdialog" aria-modal="true" aria-labelledby="cms-trash-confirm-title" aria-describedby="cms-trash-confirm-description" tabIndex={-1} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white shadow-2xl outline-none dark:border-slate-800 dark:bg-slate-900">
        ...
        <footer className="flex flex-col-reverse gap-2 border-t border-slate-200 p-4 dark:border-slate-800 sm:flex-row sm:justify-end">
          <CmsButton onClick={onClose} disabled={busy}>Hủy</CmsButton>
          <CmsButton variant="danger" leadingIcon={<Trash2 />} onClick={onConfirm} loading={busy} loadingText="Đang chuyển...">Chuyển vào Thùng rác</CmsButton>
        </footer>
      </div>
    </div>
  );
