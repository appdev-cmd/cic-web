...
interface ProjectDetailRuntimeViewProps {
  ...
  onBack?: () => void;
  onNavigateHome?: () => void;
  onNavigateToProduct?: (id: number | string) => void;
  onNavigateToService?: (id: string) => void;
  onOpenConsultation?: () => void;
}

export function ProjectDetailRuntimeView({
  project,
  onBack,
  onNavigateHome,
  onNavigateToProduct,
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {/* RIGHT COLUMN: Project Factsheet & Metadata Sidebar */}
                  ...
                  <div className="pt-4 border-t border-slate-100 space-y-2.5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {project.technologies.map((tech, idx) => (
                        <span
                          key={idx}
                          className="px-2.5 py-1 bg-slate-50 border border-slate-200 text-slate-700 text-[11px] font-semibold rounded-md flex items-center gap-1"
                        >
                          <CheckCircle2 size={11} className="text-orange-600 shrink-0" />
                          {tech}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="space-y-2">
                      {project.relatedLinks.map((link) => (
                        <div
                          key={link.id}
                          onClick={() => {
                            if (link.view === 'products' && onNavigateToProduct) {
                              onNavigateToProduct(link.id);
                            } else if (link.view === 'services' && onNavigateToService) {
                              onNavigateToService(String(link.id));
                            }
                          }}
                          className="p-3 bg-slate-50 hover:bg-orange-50/60 border border-slate-200 hover:border-orange-300 rounded-[8px] transition-all flex items-center justify-between group cursor-pointer"
                        >
