      ...
      <div className="relative group z-10">
        ...
        >
          {(renderPolicy.motionEnabled ? [...partners, ...partners] : partners).map((partner, i) => {
            const partnerIndex = partners.length > 0 ? i % partners.length : i;
            return (
              <motion.div 
                key={i}
                {...(renderPolicy.motionEnabled ? { whileHover: { scale: 1.05, y: -5 } } : {})}
                data-page-builder-media-path={JSON.stringify(['items', partnerIndex, 'logo'])}
                data-page-builder-media-id={partner.logo || ''}
              ...
              >
                {partner.logo ? (
                  <img
                    ...
                    src={partner.logo} 
                    alt={partner.name} 
                    className="max-h-12 md:max-h-14 w-full object-contain grayscale opacity-60 group-hover:opacity-100 group-hover:grayscale-0 transition-all duration-500 pointer-events-none" 
                  />
                ) : (
