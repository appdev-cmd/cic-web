...
export const CmsPagination: React.FC<CmsPaginationProps> = ({
  ...
  useEffect(() => setGoToPage(String(currentPage)), [currentPage]);
  useEffect(() => {
  ...
  const submitGoToPage = () => {
    ...
    if (!Number.isFinite(parsed)) return setGoToPage(String(currentPage));
      ...
      <div className="cms-pagination-controls">
        ...
        <div className="cms-page-navigation">
          ...
          <div className="cms-page-numbers" aria-label={`Trang ${currentPage} trên ${totalPages}`}>
            {pageItems.map((item) => typeof item === 'number' ? (
              <button key={item} type="button" onClick={() => onPageChange(item)} aria-current={item === currentPage ? 'page' : undefined} className={item === currentPage ? 'is-active' : ''}>{item}</button>
        ...
        {totalPages >= 10 && (
          <form className="cms-go-to-page" onSubmit={(event) => { event.preventDefault(); submitGoToPage(); }}>
            ...
            <input id="cms-go-to-page-input" type="number" min={1} max={totalPages} value={goToPage} onChange={(event) => setGoToPage(event.target.value)} onBlur={submitGoToPage} />
          </form>
        )}
      </div>
    </nav>
