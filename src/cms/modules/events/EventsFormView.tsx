...
export const EventsFormView: React.FC<EventsFormViewProps> = ({
  ...
  const buildCurrentEvent = (status: EditorialStatus = editorialStatus): EventItem => ({
    id: eventToEdit?.id || `ev_preview_${Date.now()}`,
    title,
    ...
    created_time: eventToEdit?.created_time || new Date().toISOString(),
    updated_time: eventToEdit?.updated_time,
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const requestedStatus = submitter?.value as EditorialStatus | undefined;
    onSave(buildCurrentEvent(requestedStatus || editorialStatus));
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {/* HEADER BAR */}
      <div className="cms-sticky-action flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-xl p-3 sm:p-4 shadow-md">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-black text-slate-900 dark:text-white mt-1">
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          <button type="button" onClick={() => onOpenPreview(buildCurrentEvent())} className="px-3 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold text-xs rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer">
            <Eye className="w-4 h-4 text-blue-500" />
            <span>Xem thử</span>
          </button>
          <button type="submit" value="draft" className="px-3.5 py-2 bg-slate-800 hover:bg-slate-900 text-white dark:bg-slate-700 dark:hover:bg-slate-600 font-bold text-xs rounded-xl flex items-center gap-1.5 shadow-sm transition-colors cursor-pointer">
            <Save className="w-4 h-4" />
            <span>Lưu bản nháp</span>
          </button>
          {editorialStatus === 'draft' && (
            <button type="submit" value="pending_review" className="px-3.5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 shadow-md shadow-blue-600/20 transition-all cursor-pointer">
              <Send className="w-4 h-4" />
              <span>Gửi duyệt</span>
            </button>
          )}
          <button type="submit" value="published" className="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 shadow-md shadow-orange-600/20 transition-all cursor-pointer">
