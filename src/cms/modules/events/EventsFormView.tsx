...
import { ArrowLeft, Calendar, FileText, Image as ImageIcon, Link2, Save, Search, Star } from 'lucide-react';
...
export const EventsFormView: React.FC<EventsFormViewProps> = ({ eventToEdit, relatedEvents, relatedArticles, relatedProducts, onSave, onCancel }) => {
  ...
  const [published, setPublished] = useState(eventToEdit?.published ?? false);
  ...
  const payload = (): Partial<EventItem> => ({
    title, chu_de: chuDe, alias: alias || slugify(title), place, time_event: timeEvent,
    ...
    tags: tagsText.split(',').map((item) => item.trim()).filter(Boolean), published,
