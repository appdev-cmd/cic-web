...
export const ProjectPreviewModal: React.FC<Props> = ({ project, onClose }) => {
  ...
  const previewViewModel: ProjectDetailViewModel = {
    id: `cms-preview-${project.id}`,
    title: project.title || 'Chưa có tiêu đề',
    slug: project.alias || `preview-${project.id}`,
    tagline: project.tagline || null,
    summary: project.summary || null,
    image: project.image || null,
    sector: project.sector || 'Dự án CIC',
    solution: project.solution || 'Đang cập nhật',
    technologies: project.technologies || [],
    customerName: project.customer_name || 'Đang cập nhật',
    ...
    seoTitle: project.seo_title || project.title || null,
    seoKeyword: project.seo_keyword || null,
    seoDescription: project.seo_description || project.summary || null,
    relatedLinks: (project.technologies || []).map((tech, idx) => ({
      id: idx + 1,
      label: tech,
      view: 'products',
  ...
  return (
    <div className="fixed inset-0 z-[90] flex flex-col bg-slate-950/85" role="dialog" aria-modal="true" aria-label="Xem trước dự án">
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-slate-700 bg-slate-900 px-5 text-white">
        <div>
          <p className="text-xs font-bold">Xem trước trên Website</p>
          <p className="text-[10px] text-slate-400">/du-an/{project.alias}</p>
        </div>
          ...
          <div className="flex rounded-lg bg-slate-800 p-1">
            {([['desktop', Monitor], ['tablet', Tablet], ['mobile', Smartphone]] as const).map(([value, Icon]) => (
              <button
                ...
                className={`rounded-md p-1.5 ${device === value ? 'bg-orange-600' : 'text-slate-400'}`}
                aria-label={`Xem ${value}`}
              >
                <Icon className="size-4" />
              </button>
            ))}
          </div>
          <button type="button" onClick={onClose} className="rounded-lg p-2 hover:bg-slate-800" aria-label="Đóng xem trước">
            <X className="size-5" />
          </button>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-auto overscroll-contain bg-slate-800 p-5">
        <ResponsiveWebsitePreviewFrame device={device}>
          ...
          <ProjectDetailRuntimeView project={previewViewModel} />
