...
import type { CmsProject, ProjectsModuleData, ProjectRelationOption } from './types';
...
export const ProjectsManager: React.FC<Props> = ({ data }) => {
  ...
  const sectors = useMemo(
    () => Array.from(new Set(projects.map((item) => item.sector).filter(Boolean))).sort(),
    [projects]
  );
  ...
  const filtered = useMemo(() => {
    return projects
      .filter((project) => {
        ...
        if (
          keyword &&
          !`${project.title} ${project.summary} ${project.customer_name} ${project.location}`
            .toLowerCase()
            .includes(keyword)
        )
          return false;
  ...
  }, [projects, query, status, sector, featured]);
  ...
  const save = async (saved: CmsProject) => {
    const isNew = saved.id.startsWith('project_');
    const payload = {
      ...saved,
      products_related: saved.products_related.map(Number).filter((n) => !isNaN(n) && n > 0),
      services_related: saved.services_related.map(Number).filter((n) => !isNaN(n) && n > 0),
    };
    ...
    try {
      if (isNew) {
        ...
        await updateProjectAction(saved.id, payload);
      }
      await fetchProjects();
  ...
  const toggleFeatured = async (id: string) => {
    ...
    if (
      !target.is_featured &&
      projects.filter((item) => item.is_featured).length >= FEATURED_CONTENT_LIMITS.project
    ) {
      return notify(
        `Đã đủ ${FEATURED_CONTENT_LIMITS.project} dự án nổi bật. Hãy bỏ chọn một dự án khác trước.`
      );
    }
    try {
      await bulkUpdateProjectsAction([id], { is_featured: !target.is_featured });
      await fetchProjects();
  ...
  const updateSelected = async (patch: { published?: boolean; is_featured?: boolean }, message: string) => {
    try {
      await bulkUpdateProjectsAction(selectedIds, patch);
      setSelectedIds([]);
      await fetchProjects();
      notify(message);
    } catch (error) {
    ...
    <div className="relative space-y-6 pb-16">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs dark:border-slate-800 dark:bg-slate-900">
        <div className="overflow-x-auto">
          <table className="cms-data-table min-w-[1050px] text-left">
            <thead>
              <tr>
              ...
              {pageItems.map((project) => (
                <tr key={project.id}>
                  ...
                  <td className="px-4 py-3 text-xs">
                    ...
                    <p className="mt-1 text-slate-500">{project.solution || '—'}</p>
                  </td>
                  <td className="px-4 py-3 text-xs">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col items-start gap-1">
                      <button
                        type="button"
                        onClick={() => void toggleFeatured(project.id)}
                        className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-bold cursor-pointer transition-colors ${
                          project.is_featured
                            ? 'bg-amber-50 text-amber-700 hover:bg-amber-100'
                            : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                        }`}
                        title={project.is_featured ? 'Bỏ nổi bật' : 'Đánh dấu nổi bật'}
                      >
                        <Star className="size-3" />
                        {project.is_featured ? 'Nổi bật' : 'Thường'}
                      </button>
                      <span
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-center gap-1">
                      <CmsIconButton
                        aria-label={`Xem trước ${project.title}`}
                        icon={<Eye />}
        ...
        {pageItems.length === 0 && (
          <div className="py-14 text-center text-sm text-slate-500">
            {loading ? 'Đang tải dữ liệu...' : 'Không tìm thấy dự án phù hợp.'}
          </div>
        )}
...
const formatPeriod = (project: CmsProject) =>
  project.start_year
    ? project.is_ongoing
