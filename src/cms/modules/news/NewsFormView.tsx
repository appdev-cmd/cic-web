...
export const NewsFormView: React.FC<NewsFormViewProps> = ({ articleToEdit, categories, relatedArticles, relatedProducts, mediaImages, placementCounts, locale, onSave, onCancel }) => {
  ...
  const [createdTime, setCreatedTime] = useState((articleToEdit?.start_time || new Date().toISOString()).slice(0, 16));
  const [endTime, setEndTime] = useState((articleToEdit?.end_time || '').slice(0, 16));
