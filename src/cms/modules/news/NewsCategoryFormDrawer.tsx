...
export const NewsCategoryFormDrawer: React.FC<Props> = ({ category, categories, onClose, onSave }) => {
  ...
  const save = () => {
    ...
    if (!slug) return setError('Đường dẫn danh mục không hợp lệ.');
    if (categories.some((item) => item.id !== form.id && item.slug === slug)) return setError('Đường dẫn này đã được một danh mục khác sử dụng.');
    onSave({ ...form, name, slug, updated_time: new Date().toISOString() });
  };
    ...
    <aside className="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col bg-white shadow-2xl dark:bg-slate-900">
      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
        ...
        <Section title="Thông tin danh mục">
          <Field label="Tên danh mục *"><input className={inputClass} value={form.name} onChange={(event) => { const name = event.target.value; setForm((current) => ({ ...current, name, slug: manualSlug ? current.slug : slugify(name) })); }} /></Field>
          ...
          <Field label="Đường dẫn"><div className="flex overflow-hidden rounded-xl border border-slate-200 focus-within:border-orange-500 dark:border-slate-700"><span className="flex items-center bg-slate-50 px-3 text-xs text-slate-500 dark:bg-slate-800">/tin-tuc/</span><input className="min-w-0 flex-1 bg-transparent px-3 py-2.5 text-sm outline-none" value={form.slug} onChange={(event) => { setManualSlug(true); setForm({ ...form, slug: slugify(event.target.value) }); }} /></div></Field>
        ...
        <Section title="Cấu hình SEO" icon={<Search className="h-4 w-4" />}>
          ...
          <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-700"><p className="text-[11px] font-bold uppercase text-slate-400">Google preview</p><p className="mt-2 text-base text-blue-700">{form.seo_title || form.title || form.name || 'Tên danh mục'}</p><p className="mt-1 break-all text-xs text-emerald-700">https://cic.com.vn/tin-tuc/{form.slug || 'duong-dan'}</p><p className="mt-2 text-xs leading-5 text-slate-600 dark:text-slate-300">{form.seo_description || form.description || 'Mô tả danh mục sẽ hiển thị tại đây.'}</p></div>
        </Section>
      </div>
...
function emptyCategory(): NewsCategory { return { id: `cat_news_${Date.now()}`, name: '', slug: '', parent_id: null, ordering: 1, published: true, show_in_homepage: true, is_hot: false, count: 0, created_time: new Date().toISOString() }; }
