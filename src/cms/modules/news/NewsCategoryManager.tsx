...
import { Edit3, Eye, EyeOff, FolderTree, Home, Plus, Search, Trash2 } from 'lucide-react';
...
export const NewsCategoryManager: React.FC<Props> = ({ categories, onChange, onMessage }) => {
  ...
  const toggle = (id: string, field: 'published' | 'show_in_homepage') => onChange(categories.map((item) => item.id === id ? { ...item, [field]: !item[field] } : item));
  ...
  return <div className="space-y-5">
    <CmsPageHeader icon={<FolderTree />} title="Danh mục tin tức" description="Quản lý cấu trúc, hiển thị và SEO của các trang danh mục tin tức." meta={<span className="rounded-md bg-orange-50 px-2 py-1 text-xs font-semibold text-orange-700">{categories.length} danh mục</span>} actions={<CmsButton size="sm" variant="primary" leadingIcon={<Plus />} onClick={() => setEditing(null)}>Thêm danh mục</CmsButton>} />
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs dark:border-slate-800 dark:bg-slate-900"><div className="relative flex items-center max-w-lg"><div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"><Search className="h-4 w-4 text-slate-400" /></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm theo tên hoặc đường dẫn..." className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-orange-500 dark:border-slate-700 dark:bg-slate-800" /></div></section>
