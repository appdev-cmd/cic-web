  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-xs dark:border-slate-800 dark:bg-slate-900 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2"><CmsButton size="sm" onClick={() => setPreview(!preview)} leadingIcon={<Eye />}>Xem trước</CmsButton><CmsButton type="submit" size="sm" variant="primary" leadingIcon={<Save />}>Lưu bản nháp</CmsButton></div>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
        <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="text-xs text-slate-500">Template chỉ quản lý nội dung. Người nhận, SMTP và routing được cấu hình ở module tương ứng.</p>
        </section>
