...
import {
  bulkSetProductBrandsPublishedAction,
  saveProductBrandAction,
  trashProductBrandAction,
} from '@/features/product-brands/server/actions';
...
export function ProductBrandsManager({
  ...
  data: ProductBrandModuleData;
  locale: ProductBrandLocale;
  capabilities: ProductBrandCapabilities;
}) {
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <CmsPageHeader
        icon={<FolderTree />}
        title="Hãng sản xuất"
        description="Quản lý danh sách hãng sản xuất."
        actions={
          capabilities.create ? (
            <CmsButton
              size="sm"
      ...
      <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <label className="relative w-full max-w-md flex-1">
            <span className="sr-only">Tìm hãng sản xuất</span>
              ...
              ? [
                  {
                    ...
                    onClick: () =>
                      runMutation(
                        () => bulkSetProductBrandsPublishedAction(locale, selectedIds, false),
                        `Đã ngừng sử dụng ${selectedIds.length} hãng.`,
                      ),
                  },
                ]
              : []
              ...
              <tr>
                <th className="sticky left-0 z-20 w-10 border-r border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-800">
                  <CmsSelectionCheckbox
                    ...
                    label="Chọn tất cả hãng trên trang"
                  />
                </th>
              ...
              {pageItems.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-12 text-center text-xs italic text-slate-400">
                    Không tìm thấy hãng sản xuất nào phù hợp với bộ lọc.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {editingItem !== undefined && (
        <BrandFormDrawer
          ...
          onSave={(payload) =>
            runMutation(
              () => saveProductBrandAction(locale, editingItem?.id ?? null, payload),
              editingItem ? 'Đã cập nhật hãng sản xuất.' : 'Đã tạo hãng sản xuất.',
            )
          }
        />
      )}
      ...
      {deletingItem && (
        <BrandDeleteDialog
          ...
          onDeactivate={() =>
            runMutation(
              () => bulkSetProductBrandsPublishedAction(locale, [deletingItem.id], false),
              `Đã ngừng sử dụng “${deletingItem.name}”.`,
            )
          }
          onTrash={() =>
            runMutation(
              () => trashProductBrandAction(locale, deletingItem.id),
...
function BrandFormDrawer({
  ...
  const titleId = 'product-brand-form-title';
  const fieldClass =
    ...
    >
      ...
      >
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h2 id={titleId} className="truncate text-base font-black text-slate-900 dark:text-white">
                {item ? 'Chỉnh sửa' : 'Thêm'} hãng sản xuất
              </h2>
        ...
        <footer className="flex flex-col-reverse gap-2 border-t border-slate-200 bg-slate-50 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] dark:border-slate-800 dark:bg-slate-900/50 sm:flex-row sm:justify-end">
          ...
          >
            {pending ? 'Đang lưu...' : item ? 'Lưu thay đổi' : 'Thêm hãng sản xuất'}
          </CmsButton>
        </footer>
      </form>
...
function BrandDeleteDialog({
  ...
  return (
    <div
      ...
      role="alertdialog"
      aria-modal="true"
      aria-labelledby="product-brand-delete-title"
      ...
      <div className="flex max-h-[calc(100dvh-2rem)] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl animate-in zoom-in-95 dark:border-slate-800 dark:bg-slate-900">
        <header className="flex items-center justify-between border-b border-slate-200 bg-red-500/10 p-5 dark:border-slate-800">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <h2 id="product-brand-delete-title" className="text-sm font-black text-slate-900 dark:text-white">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/60">
            ...
            {hasUsage && (
              <div className="mt-2 flex items-start gap-1.5 border-t border-slate-200 pt-2 font-bold text-amber-700 dark:border-slate-700 dark:text-amber-400">
                ...
                <span>Đang có {item.usageCount} sản phẩm phụ thuộc hãng này.</span>
              </div>
            )}
          </div>
            ...
            >
              ...
              <span>
                ...
                <span className="mt-0.5 block text-[11px] text-slate-500">
                  {hasUsage
                    ? 'Bị khóa vì hãng đang được dùng. Hãy đổi hãng của các sản phẩm trước.'
