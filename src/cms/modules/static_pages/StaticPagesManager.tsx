    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs dark:border-slate-800 dark:bg-slate-900">
      <div className="grid grid-cols-1 items-center gap-3 md:grid-cols-12">
        <div className="relative flex items-center md:col-span-6">
          ...
          <input value={query} onChange={(event) => { setQuery(event.target.value); setCurrentPage(1); }} placeholder="Tìm theo tên, code hoặc đường dẫn..." className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-xs font-medium outline-none focus:border-orange-500 dark:border-slate-700 dark:bg-slate-800" />
        </div>
...
function PageRow({ page, onPreview, onEdit }: { page: PageBuilderPage; onPreview: (page: PageBuilderPage) => void; onEdit: (id: string) => void }) {
  ...
  return <tr className="border-t border-slate-100 dark:border-slate-800"><td className="p-3"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400"><Globe2 className="h-4 w-4" /></span><div><p className="text-sm font-bold text-slate-900 dark:text-white">{page.name}</p><p className="text-xs text-slate-500 dark:text-slate-400">{typeLabel(page)}</p></div></div></td><td className="p-3"><p className="font-mono text-xs font-semibold text-slate-800 dark:text-slate-200">{page.code}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{page.slug}</p></td><td className="p-3 text-sm font-semibold text-slate-800 dark:text-slate-200">{page.draft.sections.length} cố định</td><td className="p-3"><span className={`rounded-md px-2 py-1 text-xs font-bold ${changed ? 'bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}`}>v{page.draft.version}{changed ? ' · Chưa publish' : ''}</span><p className="mt-1 text-[11px] text-slate-400 dark:text-slate-500">{formatTime(page.draft.updatedAt)}</p></td><td className="p-3">{page.published.version > 0 ? <span className="rounded-md bg-emerald-50 dark:bg-emerald-950/50 px-2 py-1 text-xs font-bold text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60">v{page.published.version} · Published</span> : <span className="rounded-md bg-slate-100 dark:bg-slate-800 px-2 py-1 text-xs font-bold text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">Chưa xuất bản</span>}</td><td className="p-3"><div className="flex justify-end gap-1"><button onClick={() => onPreview(page)} className="rounded-lg p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-orange-600 dark:hover:text-orange-400 cursor-pointer" title="Preview Draft"><Eye className="h-4 w-4" /></button><button onClick={() => onEdit(page.id)} className="rounded-lg p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-orange-600 dark:hover:text-orange-400 cursor-pointer" title="Chỉnh sửa"><Edit className="h-4 w-4" /></button></div></td></tr>;
}

function PageCard({ page, onPreview, onEdit }: { page: PageBuilderPage; onPreview: (page: PageBuilderPage) => void; onEdit: (id: string) => void }) {
  return <article className="space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400"><Globe2 className="h-4 w-4" /></span><div className="min-w-0"><h2 className="truncate text-sm font-bold text-slate-900 dark:text-white">{page.name}</h2><p className="text-xs text-slate-500 dark:text-slate-400">{typeLabel(page)}</p></div></div><div className="flex shrink-0 gap-1"><button onClick={() => onPreview(page)} className="rounded-lg bg-slate-100 dark:bg-slate-800 p-2 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer" aria-label={`Xem trước ${page.name}`}><Eye className="h-4 w-4" /></button><button onClick={() => onEdit(page.id)} className="rounded-lg bg-orange-50 dark:bg-orange-950/50 p-2 text-orange-600 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-900/50 cursor-pointer" aria-label={`Chỉnh sửa ${page.name}`}><Edit className="h-4 w-4" /></button></div></div><div className="rounded-lg bg-slate-50 dark:bg-slate-800/60 px-3 py-2"><p className="break-all font-mono text-xs font-semibold text-slate-800 dark:text-slate-200">{page.slug}</p><p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">{page.draft.sections.length} section cố định · Draft v{page.draft.version}{page.published.version > 0 ? ` · Published v${page.published.version}` : ' · Chưa xuất bản'}</p></div></article>;
}
...
function CreatePageModal({ name, slug, onNameChange, onClose, onCreate }: { name: string; slug: string; onNameChange: (value: string) => void; onClose: () => void; onCreate: () => void }) {
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-xs" role="dialog" aria-modal="true" aria-label="Tạo trang nội dung"><div className="w-full max-w-lg rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-2xl border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100"><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-black text-slate-900 dark:text-white">Tạo trang nội dung</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Dùng cùng thiết kế với Chính sách bảo mật và Điều khoản sử dụng.</p></div><button onClick={onClose} className="rounded-lg p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer" aria-label="Đóng"><X className="h-5 w-5" /></button></div><div className="mt-5 space-y-4"><label className="block space-y-1.5"><span className="text-xs font-bold text-slate-700 dark:text-slate-300">Tên trang *</span><input value={name} onChange={(event) => onNameChange(event.target.value)} className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white px-3 py-2.5 text-sm outline-none focus:border-orange-500" placeholder="Ví dụ: Quy chế hoạt động" /></label><div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 p-3"><p className="text-[11px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Đường dẫn tự động</p><p className="mt-1 break-all font-mono text-sm font-semibold text-slate-800 dark:text-slate-100">/{slug}</p><p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">Nếu đã tồn tại, hệ thống tự thêm hậu tố ngắn như -2, -3.</p></div><div className="break-all rounded-xl bg-orange-50 dark:bg-orange-950/40 border border-orange-200 dark:border-orange-900/50 p-3 text-xs text-orange-800 dark:text-orange-300"><span className="font-bold">Link sau khi xuất bản:</span> https://cic.com.vn/{slug}</div></div><div className="mt-6 flex justify-end gap-2"><CmsButton variant="secondary" onClick={onClose}>Hủy</CmsButton><CmsButton onClick={onCreate} disabled={!name.trim()}>Tạo Draft</CmsButton></div></div></div>;
}
