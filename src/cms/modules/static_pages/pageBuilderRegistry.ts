...
export const sectionDefinitions: Record<string, SectionDefinition> = {
  ...
  'about.hero': { label: 'Hero Giới thiệu', description: 'Tiêu đề và ảnh mở đầu.', editableContract: blockedContract('about.hero', { media: { background: { path: 'backgroundImageId', semantic: 'background-image', ownership: 'static-unwired', replace: 'blocked', blockedReason: 'data-unwired' } } }) },
  'about.overview': { label: 'Tổng quan doanh nghiệp', description: 'Giới thiệu và video doanh nghiệp.', editableContract: blockedContract('about.overview', { media: { image: { path: 'imageId', semantic: 'image', ownership: 'static-unwired', replace: 'blocked', blockedReason: 'data-unwired' }, video: { path: 'videoUrl', semantic: 'video', ownership: 'static-unwired', replace: 'blocked', optional: true, blockedReason: 'data-unwired' } } }) },
  'about.timeline': {
    ...
    editableContract: {
      ...
      fields: [
        ...
        { path: 'description', semantic: 'text', ownership: 'static-unwired', valueKind: 'string', editing: 'blocked', blockedReason: 'data-unwired' },
  ...
  'about.offerings': { label: 'Sản phẩm và dịch vụ cung cấp', description: 'Các entity được chọn cho grid cố định.', referenceLimit: { product: 2, service: 4 }, editableContract: blockedContract('about.offerings') },
  'about.awards': { label: 'Thành tựu & Giải thưởng', description: 'Danh sách giải thưởng.', editableContract: blockedContract('about.awards', { media: { images: { path: 'items.*.imageId', semantic: 'image', ownership: 'embedded', replace: 'blocked', blockedReason: 'identity-unresolved' } } }) },
  'about.partners': { label: 'Đối tác chiến lược', description: 'Danh sách đối tác.', referenceLimit: { partner: 12 }, editableContract: blockedContract('about.partners') },
