...
export async function ProductCategoriesRoute() {
  const principal = await requireCmsPageAccess();
  if (!can(principal, 'product_settings', 'view')) {
    return <div className="rounded-2xl border border-amber-200 bg-amber-50 p-8 text-center text-sm font-semibold text-amber-800">Bạn không có quyền xem Danh mục sản phẩm.</div>;
  }
  const data = await getProductCategoryModuleData();
  return <ProductCategoriesScreen data={data} capabilities={{
