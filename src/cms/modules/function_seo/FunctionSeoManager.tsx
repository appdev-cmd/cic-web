...
export const FunctionSeoManager: React.FC<Props> = ({ workspaceLocale }) => {
  ...
  const save = () => {
    ...
    setEditing(null);
    setToast('Đã lưu SEO trang chính. SEO danh mục và chi tiết không bị thay đổi.');
  ...
  return <div className="space-y-5">
    ...
    <CmsPageHeader icon={<SearchCheck />} title="Cấu hình SEO chức năng" description="Quản lý SEO trang chính của các module và xem rõ nơi quản lý SEO danh mục, nội dung chi tiết." meta={<span className="rounded-md bg-orange-50 px-2 py-1 text-xs font-bold text-orange-700">{workspaceLocale.toUpperCase()} · {records.length} module</span>} />

    <section className="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-900 dark:bg-blue-950/30"><div className="flex gap-3"><FolderTree className="mt-0.5 h-5 w-5 shrink-0 text-blue-600" /><div><h2 className="text-sm font-bold text-blue-900 dark:text-blue-100">Mỗi module có ba cấp SEO độc lập</h2><div className="mt-3 flex flex-wrap items-center gap-2 text-xs font-semibold text-blue-800 dark:text-blue-200"><span className="rounded-lg bg-white px-3 py-2 shadow-sm dark:bg-slate-900">1. Trang chính</span><ChevronRight className="h-4 w-4" /><span className="rounded-lg bg-white px-3 py-2 shadow-sm dark:bg-slate-900">2. Danh mục</span><ChevronRight className="h-4 w-4" /><span className="rounded-lg bg-white px-3 py-2 shadow-sm dark:bg-slate-900">3. Chi tiết</span></div><p className="mt-3 text-xs leading-5 text-blue-700 dark:text-blue-300">Module này chỉ sửa cấp 1. Cấp 2 và 3 vẫn được chỉnh tại module sở hữu để không trùng dữ liệu hoặc target search intent.</p></div></div></section>
    ...
    <div className="space-y-3">{filtered.map((item) => {
      ...
      return <section key={item.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        {expanded && <div className="border-t border-slate-200 p-4 dark:border-slate-800 sm:p-5"><div className="relative space-y-3 before:absolute before:bottom-6 before:left-[19px] before:top-6 before:w-px before:bg-slate-200 dark:before:bg-slate-700">
          ...
          <SeoLevel number="2" icon={<FolderTree />} title="Trang danh mục" pattern={item.categoryPattern} owner={item.categoryOwner} status={item.categoryStatus} href={item.categoryPath} />
          <SeoLevel number="3" icon={<FileText />} title="Trang chi tiết" pattern={item.detailPattern} owner={item.detailOwner} status={item.detailStatus} href={item.detailPath} />
        </div></div>}
      </section>;
    })}</div>
