...
--   fs_menus_items_en_bk — Bản backup thủ công của fs_menus_items_en (hậu tố _bk) — không phải bảng nghiệp vụ chính thức.
--   fs_contents_en-bk — Bản backup thủ công của fs_contents_en (hậu tố -bk) — trùng lặp dữ liệu với bảng gốc.
--   fs_news_18052020 — Bản backup/snapshot của fs_news chụp tại một thời điểm (18/05/2020) — mang tính lịch sử, không cập nhật theo thời gian thực.
--   fs_news_categories_copy1 — Bản sao nháp của fs_news_categories (hậu tố _copy1), khả năng cao được tạo trong lúc thử nghiệm/sửa lỗi.
--   fs_news_en1 — Bản trùng lặp của fs_news_en (hậu tố "1"), không theo quy ước đặt tên chuẩn của hệ thống.
--   fs_products_copy1 — Bản sao nháp của fs_products (hậu tố _copy1).
--   fs_products_en_ — Bản trùng lặp/lỗi đặt tên của fs_products_en (thừa dấu gạch dưới cuối tên).
--   fs_manufactories_en_bk — Bản backup thủ công của fs_manufactories_en.
--   fs_banners_en_ — Bản trùng lặp/lỗi đặt tên của fs_banners_en.
--   fs_image_en_ — Bản trùng lặp/lỗi đặt tên của fs_image_en.
--   fs_image_images_en_ — Bản trùng lặp/lỗi đặt tên của fs_image_images_en.
...
CREATE TABLE "cic_address" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_address.id / fs_address_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_areas" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_areas.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_cities" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_cities.id / fs_cities_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_regions" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_khuvuc.id / fs_khuvuc_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_wards" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_wards.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_users" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_users.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_users_permission" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_users_permission.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_users_permission_field" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_users_permission_field.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_users_permission_fun" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_users_permission_fun.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_permission_field" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_permission_field.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_permission_fun" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_permission_fun.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_permission_tasks" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_permission_tasks.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_members" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_members.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_menus_admin" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_menus_admin.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_menus_createlink" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_menus_createlink.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_menus_groups" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_menus_groups.id / fs_menus_groups_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_menus_items" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_menus_items.id / fs_menus_items_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_config" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_config.id / fs_config_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_config_enjicad" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_config_enjicad.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_config_modules" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_config_modules.id / fs_config_modules_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_blocks" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_blocks.id / fs_blocks_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_blocks_translations" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- Khoá chính riêng của dòng bản dịch (không dùng chung id với bảng gốc nữa).
...
CREATE TABLE "cic_blocks_exist" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_blocks_exist.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_tables" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_tables.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_extends_groups" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_extends_groups.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_extends_items" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_extends_items.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_history" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_history.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_hits" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_hits.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_languages" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_languages.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_languages_contents" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_languages_contents.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_languages_tables" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_languages_tables.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_languages_text" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_languages_text.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_languages_text_admin" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_languages_text_admin.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_translate_content" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_translate_content.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_contents_categories" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_contents_categories.id / fs_contents_categories_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_news_keyword" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_news_keyword.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_keywords" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_keywords.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_event" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_event.id / fs_event_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_products" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_products.id / fs_products_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_products_fields_groups" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_products_fields_groups.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_products_filters" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_products_filters.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_products_filters_values" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_products_filters_values.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_products_incentives" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_products_incentives.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_products_price" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_products_price.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_products_sizes" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_products_sizes.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_products_tables" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_products_tables.id / fs_products_tables_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_products_types" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_products_types.id / fs_products_types_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_product_contact" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_product_contact.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_manufactories" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_manufactories.id / fs_manufactories_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_order" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_order.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_order_items" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_order_items.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_banners_categories" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_banners_categories.id / fs_banners_categories_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_slideshow" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_slideshow.id / fs_slideshow_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_slideshow_categories" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_slideshow_categories.id / fs_slideshow_categories_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_contact" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_contact.id / fs_contact_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_contact_enjicad" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_contact_enjicad.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_email" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_email.id / fs_email_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_types_email" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_types_email.id / fs_types_email_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_onlinesupport" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_onlinesupport.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_business" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_business.id / fs_business_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
  "code" varchar(255), -- ← fs_business.code | Mã định danh dạng chuỗi (mã code) của đối tượng.
  "tablenames" varchar(255), -- ← fs_business.tablenames | Tên các bảng liên kết (có thể nhiều bảng).
  "published" boolean, -- ← fs_business.published | Đổi tinyint(4) dạng cờ 0/1 sang boolean thật của Postgres; khuyến nghị NOT NULL DEFAULT.
  "ordering" integer, -- ← fs_business.ordering | Thứ tự sắp xếp hiển thị.
  "created_time" timestamptz, -- ← fs_business.created_time | Đổi datetime → timestamptz; nên thêm DEFAULT now().
  "image" varchar(255), -- ← fs_business.image | Đường dẫn ảnh chính.
  "first_toll" varchar(255), -- ← fs_business.first_toll | Chi phí/mức phí ban đầu.
  "show_in_homepage" boolean, -- ← fs_business.show_in_homepage | Đổi tinyint(4) dạng cờ 0/1 sang boolean thật của Postgres; khuyến nghị NOT NULL DEFAULT.
  "prefix_name" varchar(255), -- ← fs_business.prefix_name | Tiền tố gắn trước tên.
  "old_id" integer, -- ← fs_business.old_id | Giữ tạm để đối chiếu dữ liệu trong lúc migrate; có thể xoá sau khi xác nhận không còn tham chiếu.
  "updated_time" timestamptz, -- ← fs_business.updated_time | Đổi datetime → timestamptz; nên có trigger auto-update khi UPDATE row.
  "color_code" varchar(255), -- ← fs_business.color_code | Mã màu.
  "is_retail" boolean, -- ← fs_business.is_retail | Đổi tinyint(4) dạng cờ 0/1 sang boolean thật của Postgres; khuyến nghị NOT NULL DEFAULT.
  "is_common" boolean, -- ← fs_business.is_common | Đổi tinyint(4) dạng cờ 0/1 sang boolean thật của Postgres; khuyến nghị NOT NULL DEFAULT.
  "phone" varchar(225), -- ← fs_business.phone | Số điện thoại.
  "Skype" varchar(255), -- ← fs_business.Skype | Tài khoản Skype liên hệ.
  "Zalo" varchar(255), -- ← fs_business.Zalo | Số điện thoại/tài khoản Zalo liên hệ.
  "region" varchar(255), -- ← fs_business.khuvuc | ✅ [MỚI] Đổi tên khuvuc→region (tiếng Anh, hoàn tất phần đã hoãn khi đổi tên bảng khuvuc→regions). Khu vực. Lưu ý: đang là văn bản tự do (varchar), KHÔNG phải khoá ngoại tới bảng regions(id) — chưa xác nhận nghiệp vụ có nên chuẩn hoá thành FK hay không, tạm giữ nguyên dạng text.
  "region_name" varchar(255), -- ← fs_business.khuvuc_name | ✅ [MỚI] Đổi tên khuvuc_name→region_name (tiếng Anh, hoàn tất phần đã hoãn khi đổi tên bảng khuvuc→regions). Tên khu vực.
  "products" varchar(255), -- ← fs_business.products | Tên/mã sản phẩm liên kết.
  "lienhe" text, -- ← fs_business.lienhe | Thông tin liên hệ.
  "lienhe_kd" text, -- ← fs_business.lienhe_kd | Thông tin liên hệ bộ phận kinh doanh.
  "lienhe_kt" text, -- ← fs_business.lienhe_kt | Thông tin liên hệ bộ phận kỹ thuật.
  "lienhe_kdmb" text, -- ← fs_business.lienhe_kdmb | Thông tin liên hệ kinh doanh miền Bắc.
  "lienhe_kdmn" text, -- ← fs_business.lienhe_kdmn | Thông tin liên hệ kinh doanh miền Nam.
  "name" varchar(255), -- ← fs_business.name + fs_business_en.name | Nội dung theo ngôn ngữ — nằm trực tiếp trong bảng độc lập (không tách bảng dịch riêng, theo quyết định tách VI/EN độc lập).
  "alias" varchar(255), -- ← fs_business.alias + fs_business_en.alias | Nội dung theo ngôn ngữ — nằm trực tiếp trong bảng độc lập (không tách bảng dịch riêng, theo quyết định tách VI/EN độc lập).
  "description" text, -- ← fs_business.description + fs_business_en.description | Nội dung theo ngôn ngữ — nằm trực tiếp trong bảng độc lập (không tách bảng dịch riêng, theo quyết định tách VI/EN độc lập).
  "seo_title" varchar(255), -- ← fs_business.seo_title + fs_business_en.seo_title | Nội dung theo ngôn ngữ — nằm trực tiếp trong bảng độc lập (không tách bảng dịch riêng, theo quyết định tách VI/EN độc lập).
  "seo_keyword" varchar(255), -- ← fs_business.seo_keyword + fs_business_en.seo_keyword | Nội dung theo ngôn ngữ — nằm trực tiếp trong bảng độc lập (không tách bảng dịch riêng, theo quyết định tách VI/EN độc lập).
  "seo_description" varchar(255), -- ← fs_business.seo_description + fs_business_en.seo_description | Nội dung theo ngôn ngữ — nằm trực tiếp trong bảng độc lập (không tách bảng dịch riêng, theo quyết định tách VI/EN độc lập).
  "content" text -- ← fs_business.content + fs_business_en.content | Nội dung theo ngôn ngữ — nằm trực tiếp trong bảng độc lập (không tách bảng dịch riêng, theo quyết định tách VI/EN độc lập).
);
...
CREATE TABLE "cic_services" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_services.id / fs_services_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
  "content" text, -- ← fs_services.content | Nội dung chi tiết (thường là HTML).
  "tags" varchar(255), -- ← fs_services.tags | Thẻ gắn với nội dung (dùng để phân loại/tìm kiếm).
  "category_id" integer, -- ← fs_services.category_id | ⚠️ THIẾU BẢNG THAM CHIẾU — ⚠ CẦN QUYẾT ĐỊNH NGHIỆP VỤ — chỉ 4 dòng, tất cả cùng category_id = 2, không có bảng services_categories tương ứng; xác nhận với nghiệp vụ trước khi bỏ cột hoặc tạo bảng danh mục dịch vụ.
  "category_alias" varchar(255), -- ← fs_services.category_alias | Alias của danh mục.
  "category_id_wrapper" varchar(255), -- ← fs_services.category_id_wrapper | Mã danh mục cha bao ngoài (dùng cho breadcrumb/URL lồng nhau).
  "category_alias_wrapper" varchar(255), -- ← fs_services.category_alias_wrapper | Alias bao ngoài của danh mục cha.
  "category_published" integer, -- ← fs_services.category_published | Trạng thái hiển thị của danh mục.
  "image" varchar(255), -- ← fs_services.image | Đường dẫn ảnh chính.
  "creator" varchar(255), -- ← fs_services.creator | Người tạo bản ghi.
  "source_website" varchar(255), -- ← fs_services.source_website | Nguồn website.
  "created_time" timestamptz, -- ← fs_services.created_time | Đổi datetime → timestamptz; nên thêm DEFAULT now().
  "updated_time" timestamptz, -- ← fs_services.updated_time | Đổi datetime → timestamptz; nên có trigger auto-update khi UPDATE row.
  "editor" varchar(255), -- ← fs_services.editor | Người chỉnh sửa gần nhất.
  "show_in_homepage" boolean, -- ← fs_services.show_in_homepage | Đổi tinyint(4) dạng cờ 0/1 sang boolean thật của Postgres; khuyến nghị NOT NULL DEFAULT.
  "hits" integer, -- ← fs_services.hits | Tổng lượt xem/truy cập.
  "published" boolean, -- ← fs_services.published | Đổi tinyint(4) dạng cờ 0/1 sang boolean thật của Postgres; khuyến nghị NOT NULL DEFAULT.
  "ordering" integer, -- ← fs_services.ordering | Thứ tự sắp xếp hiển thị.
  "display_title" boolean, -- ← fs_services.display_title | Đổi tinyint(4) dạng cờ 0/1 sang boolean thật của Postgres; khuyến nghị NOT NULL DEFAULT.
  "display_column" integer, -- ← fs_services.display_column | Cột được chọn để hiển thị.
  "tags_group" integer, -- ← fs_services.tags_group | Nhóm thẻ (tags).
  "rating_count" integer, -- ← fs_services.rating_count | Số lượt đánh giá.
  "rating_sum" integer, -- ← fs_services.rating_sum | Tổng điểm đánh giá (dùng để tính điểm trung bình).
  "source" varchar(255), -- ← fs_services.source | Nguồn dữ liệu/nguồn trích dẫn.
  "show_map" boolean, -- ← fs_services.show_map | Đổi tinyint(4) dạng cờ 0/1 sang boolean thật của Postgres; khuyến nghị NOT NULL DEFAULT.
  "author" varchar(255), -- ← fs_services.author | Tên tác giả nội dung.
  "author_last" varchar(255), -- ← fs_services.author_last | Tên người chỉnh sửa gần nhất.
  "tawk_to" text, -- ← fs_services.tawk_to | Mã tích hợp chat trực tuyến Tawk.to.
...
CREATE TABLE "cic_image" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_image.id / fs_image_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_image_images" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_image_images.id / fs_image_images_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_video" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_video.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_application" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_application.id / fs_application_en.id | Khoá chính tự tăng của bảng gốc, dùng identity thay AUTO_INCREMENT.
...
CREATE TABLE "cic_year" (
  "id" integer PRIMARY KEY GENERATED ALWAYS AS IDENTITY, -- ← fs_year.id | Khoá chính tự tăng, dùng identity thay AUTO_INCREMENT.
