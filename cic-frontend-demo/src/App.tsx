...
import { BIMIcon, ZaloIcon } from './components/shared/Icons';
import { Counter } from './components/shared/Counter';

// Import centralized mock data & types
import { 
  heroSlides, 
  projects, 
  newsItems, 
...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'home' | 'products' | 'about' | 'services' | 'projects' | 'news' | 'events' | 'contact' | 'privacy' | 'terms'>('home');
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-orange-500 selection:text-white relative">
      ...
      {currentView === 'home' ? (
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-y-12 gap-x-8 md:divide-x divide-slate-200">
                {[
                  ...
                  { isIcon: true, icon: <ShieldCheck size={44} className="mx-auto text-orange-600" />, label: 'Phần mềm bản quyền' },
                ].map((stat, i) => (
                  <motion.div 
                    key={i}
          ...
          <section id="solutions" className="py-12 bg-slate-50/40  text-slate-950 relative overflow-hidden z-10">
            <div className="max-w-7xl mx-auto px-6 relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
                ...
                {/* Grid row 2 */}
                {[
                  { icon: <Cpu size={24} />, title: 'Thiết bị & IoT', desc: 'Hệ thống đo đạc trắc địa, sensor quan trắc địa kỹ thuật, cảm biến môi trường thông minh và thiết bị kiểm định chất lượng công trình.' },
                  { icon: <Layers size={24} />, title: 'Phần mềm Kỹ thuật', desc: 'Cung cấp và chuyển giao hệ thống phần mềm chuyên sâu chính hãng cho phân tích kết cấu, cầu đường, thủy lợi và địa chất.' },
                  { icon: <Users size={24} />, title: 'Đào tạo & Chuyển giao', desc: 'Tổ chức các khóa đào tạo nâng cao năng lực cho đội ngũ kỹ sư, hỗ trợ kỹ thuật tận tâm và cấp chứng chỉ chuẩn quốc tế.' }
                ].map((sol, i) => (
                  <motion.div 
                    key={i}
                    initial={{ opacity: 0, y: 40 }}
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
              ...
              {activeEventTab === 'upcoming' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {[
                    { title: 'Hội thảo: Ứng dụng AI và Digital Twins trong vận hành hạ tầng số', date: '25/06/2026', time: '08:30 - 11:30', loc: 'Khách sạn JW Marriott, Hà Nội' },
                    { title: 'Khóa Đào Tạo: Quy Trình Phối Hợp BIM Cho Quản Lý Dự Án Xây Dựng', date: '02/07/2026', time: '09:00 - 16:30', loc: 'Trung tâm Đào tạo CIC, TP.HCM' }
                  ].map((evt, i) => (
                    <motion.div 
                      key={i}
                      initial={{ opacity: 0, y: 20 }}
                      ...
                      viewport={{ once: true }}
                      transition={{ delay: i * 0.1 }}
                      className="bg-white border border-slate-200 p-8 flex flex-col md:flex-row gap-6 relative group hover:border-orange-600 hover:shadow-2xl transition-all duration-300"
                    >
                      <div className="w-16 h-16 bg-orange-600/10 text-orange-600 flex flex-col items-center justify-center font-black uppercase tracking-wider shrink-0 rounded-none">
                        ...
                        <span className="text-[10px]">Th6</span>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {[
                    { title: 'Bentley Innovation Day 2024', date: '15/04/2024', attendees: '500+ Khách mời' },
                    { title: 'Hội thảo chuyên sâu Tekla Structures', date: '28/03/2024', attendees: '200+ Doanh nghiệp' },
                    { title: 'Lễ ra mắt enjiCAD thế hệ mới', date: '12/03/2024', attendees: '300+ Kỹ sư' }
                  ].map((evt, i) => (
                    <motion.div 
                      key={i}
                      initial={{ opacity: 0, y: 20 }}
                      whileInView={{ opacity: 1, y: 0 }}
                      viewport={{ once: true }}
                      className="bg-white border border-slate-200 p-6 flex flex-col justify-between group hover:border-orange-600 hover:shadow-lg transition-all duration-300"
                    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {filteredNews.map((news, i) => (
                  <motion.div 
                    ...
                    viewport={{ once: true }}
                    transition={{ delay: i * 0.1 }}
                    className="bg-white border border-slate-200 group flex flex-col hover:border-orange-600 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300"
                  >
        ...
        <ServicesView initialServiceId={activeServiceId} onNavigateHome={() => { setCurrentView('home'); setActiveLink('Giới thiệu'); }} />
