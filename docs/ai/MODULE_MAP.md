...
| `[I]` | Events | `/events`, `/events/[slug]` | Events | event schema/query/mapper; media; published visibility | news, products, forms/customer requests, email | Core DB-backed: danh sách, chi tiết, quan hệ sản phẩm/tin tức, lọc trạng thái, nối EventsRuntimeView trực tiếp DB thật; registration/email còn pending |
| `[I]` | Contact/consultation | `/contact`, global consultation | Contacts, CTA, Forms, Customer Requests | form/CTA contract; validated submission; anti-spam/rate limit; persistence | staff assignment, email template/delivery, audit | Hoàn tất tích hợp 2026-09-18: Form liên hệ hệ thống kết nối DB PostgreSQL thật (cic_contact & cic_contact_en), tự động đồng bộ sang operational overlay cic_customer_request_states và cic_customer_request_events; Tự động kích hoạt gửi email thông báo cho Admin qua Google Workspace SMTP (nampt@cic.com.vn) và email xác nhận cho Khách hàng; Soft pending: anti-spam rate limiting nâng cao |
...
| `[I]` | Contacts/CRM Inbox | contact requests, PII, assignment, spam/duplicate | auth/RBAC + PII scope; contact schema; submission persistence | users/staff, services, audit, email | Hoàn tất tích hợp 2026-09-18: Hợp nhất Hộp thư liên hệ vào module Yêu cầu khách hàng (/cms/customer-requests), chuẩn hóa route /cms/contacts và /cms/contact-requests trong routing.ts; Đã backfill 348 bản ghi liên hệ lịch sử vào cic_customer_request_states; Phân trang, tìm kiếm, lọc theo nguồn liên hệ, cập nhật trạng thái/độ ưu tiên, ghi chú nội bộ và phân công nhân sự hoạt động trực tiếp trên DB PostgreSQL thật |
