...
| `[I]` | Product Settings | categories, brands, applications, types, sales owners | auth/RBAC; approved taxonomy tables/relations | products usage impact, users/sales scope | Core DB-backed: 5 bộ dữ liệu phân loại (danh mục, hãng sản xuất, lĩnh vực ứng dụng, loại sản phẩm, phụ trách kinh doanh) đã kết nối PostgreSQL thật qua các routes riêng biệt kèm Server Actions (CRUD/publish/trash); usage impact và sales scope liên kết sâu còn pending |
