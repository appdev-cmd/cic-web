...
|---|---|---|---|---|---|---|
| `[x]` | Home | `/`, hero, ecosystem, highlights, stats | Static Pages/Page Builder, config | public shell; published page/config read model | products, services, projects, news, events, partners, CTA | Hoàn tất implementation 2026-09-21: Trang chủ nạp trực tiếp snapshot `published_revision_id` từ `cic_content_pages` (Page 1 cho VI, Page 7 cho EN) thông qua `getPublishedHomePage()`; Phân giải đầy đủ 10 sections thật (Hero, Intro, Stats, Awards, Ecosystem, Projects, Events, News, Partners, Contact CTA); Tự động nạp real entities từ `cic_projects`, `cic_event`, `cic_news`, `cic_manufactories`; Cache in-memory kết hợp revalidation tức thì khi CMS xuất bản; Hỗ trợ song ngữ VI/EN độc lập; Roundtrip test pass 100% |
| `[x]` | About/company/partners | `/about`, `/gioi-thieu/*` | Static Pages/Page Builder | public shell; published about snapshot | products, services, media, partner assets, awards | Hoàn tất implementation 2026-09-21: Đồng bộ 100% với DB PostgreSQL thật qua `aboutResolver.ts` cho cả 3 trang con (`about` - 8 sections, `organization` - 2 sections, `capacity_experience` - 4 sections); Chuẩn hóa slug tiếng Việt `/gioi-thieu` và tiếng Anh `/about`; Tích hợp flipbook PDF hồ sơ năng lực, video YouTube popover và form tư vấn popup gửi về Yêu cầu khách hàng; SEO metadata động theo từng trang; Roundtrip test pass 100% |
| `[x]` | Products | `/products`, `/products/[slug]` | Products + Product Settings | product schema/query/mapper; taxonomy; media; published visibility | projects, services, CTA/forms, SEO | Hoàn tất implementation 2026-09-21: Nạp trực tiếp từ PostgreSQL thật (cic_products & cic_products_en); Hỗ trợ đầy đủ bộ lọc danh mục, lĩnh vực ứng dụng, loại sản phẩm, phụ trách kinh doanh; Chi tiết sản phẩm nạp đầy đủ thông số, tài liệu tải về, media, quan hệ sản phẩm; Form/modal tư vấn báo giá nối submitCustomerInteraction lưu DB và gửi email tự động; Hỗ trợ song ngữ VI/EN độc lập; Roundtrip test pass 100% |
| `[x]` | Services | `/services`, `/services/[slug]` | Services | service schema/query/mapper; media; published visibility | products, projects, contacts, SEO | Hoàn tất implementation 2026-09-21: Danh sách và chi tiết dịch vụ nạp từ cic_services & cic_services_en; Hiển thị quy trình thực hiện, dịch vụ/sản phẩm liên quan; Form yêu cầu tư vấn dịch vụ tích hợp submitCustomerInteraction lưu DB thật và gửi email tự động; Hỗ trợ song ngữ VI/EN độc lập; Roundtrip test pass 100% |
| `[x]` | Projects | `/projects`, `/projects/[slug]` | Projects | project schema/query/mapper; media; published visibility | products, services, CTA, SEO | Hoàn tất implementation 2026-09-21: Danh sách portfolio 2 cột bất đối xứng 7-5/5-7 nạp từ cic_projects & cic_projects_en; Chi tiết dự án factsheet, công nghệ ứng dụng, quan hệ sản phẩm/dịch vụ, 3 dự án liên quan cùng sector; Modal tư vấn dự án nối DB thật; Hỗ trợ song ngữ VI/EN độc lập; Roundtrip test pass 100% |
| `[x]` | News | `/news`, `/news/[slug]` | News + Categories | news/category schema; published query; media; rich HTML policy | public EN locale routing; Home/Header binding; authenticated visual regression | Hoàn tất implementation 2026-09-21: Danh sách tin tức nạp từ cic_news & cic_news_en; Phân trang, lọc theo danh mục, bài viết tiêu biểu; Chi tiết bài viết chuẩn hóa SEO, sanitized HTML, tin liên quan, chia sẻ mạng xã hội; Hỗ trợ song ngữ VI/EN độc lập; Roundtrip test pass 100% |
| `[x]` | Events | `/events`, `/events/[slug]` | Events | event schema/query/mapper; media; published visibility | news, products, forms/customer requests, email | Hoàn tất implementation 2026-09-21: Core DB-backed danh sách, chi tiết, quan hệ sản phẩm/tin tức, lọc trạng thái; Form đăng ký vé tham dự registerEventAction nối DB thật, tự động đồng bộ sang Yêu cầu khách hàng và gửi email xác nhận cho khách hàng + thông báo Admin qua Google Workspace SMTP; Roundtrip test pass 100% |
| `[x]` | Contact/consultation | `/contact`, global consultation | Contacts, CTA, Forms, Customer Requests | form/CTA contract; validated submission; anti-spam/rate limit; persistence | staff assignment, email template/delivery, audit | Hoàn tất implementation 2026-09-21: Form liên hệ hệ thống kết nối DB PostgreSQL thật (cic_contact & cic_contact_en), tự động đồng bộ sang operational overlay cic_customer_request_states và cic_customer_request_events; Tự động kích hoạt gửi email thông báo cho Admin qua Google Workspace SMTP (nampt@cic.com.vn) và email xác nhận cho Khách hàng; Form validation & anti-spam captcha pass |
| `[x]` | Public search | `/search` | Search projection | published read models của các domain được index; visibility/locale | ranking, SEO analytics | Hoàn tất implementation 2026-09-21: Tuyến đường /search nạp trực tiếp kết quả tìm kiếm đa miền (sản phẩm, dịch vụ, dự án, tin tức) từ PostgreSQL thật thông qua searchPublishedContent(); Hỗ trợ lọc theo từ khóa, hiển thị nhãn phân loại, tóm tắt và liên kết chi tiết; Roundtrip test pass 100% |
| `[x]` | Menu/navigation | Header, Footer, route links | Menu (`cic_menus_groups*`, `cic_menus_items*`) | route registry; published menu query; locale | referenced content visibility | Hoàn tất implementation 2026-09-17: Seed & chuẩn hóa toàn bộ dữ liệu menu canonical cho cả VI và EN; Header (desktop navbar + mobile drawer) và Footer (Quick links, Solutions, Services) đọc trực tiếp từ PostgreSQL thật qua `getNavigationDataFromDb`; Loại bỏ hoàn toàn phụ thuộc vào fixture mockData; Tích hợp Server Component public layout song song; Roundtrip pass 100% |
| `[x]` | Legal/static content | `/privacy`, `/terms`, `/chinh-sach-bao-mat`, `/dieu-khoan-su-dung`, `/[slug]` | Static Pages (`cic_content_pages*`, template: `legal_standard`) | public shell; published legal snapshot | SEO, localization, trash for custom pages | Hoàn tất implementation 2026-09-21: Seed & publish chuẩn hóa toàn bộ 4 trang hệ thống (Privacy & Terms VI/EN); PublicLegalPageView chuyển hoàn toàn sang Server Component chuẩn UX/SEO parity; Xử lý alias routing 2 chiều; Dynamic [slug] hỗ trợ trang quy chế/nội dung chuẩn mới do admin tạo; Visual Canvas CMS đồng bộ 1:1; Roundtrip test pass 100% |
| `[A]` | Public widgets | chatbot, floating contact, consultation | Config, CTA/Forms/Requests | public shell; approved config/submission boundary | external webhook/chat provider, audit | Đang tạm dừng chờ API chính thức của đối tác thứ ba theo yêu cầu người dùng; các nút gọi hotline/zalo hiện hữu vẫn duy trì fallback trực tiếp |

## CMS/business modules

| Status | Domain/module | CMS quản lý | Hard dependency | Soft/integration dependency | Hiện trạng quan sát |
|---|---|---|---|---|---|
| `[x]` | Dashboard | aggregates, notifications, work queues | CMS auth/RBAC; read models từ domain nguồn | analytics, audit, workflow queues | Hoàn tất implementation 2026-09-21: PostgreSQL aggregate read-model thật 100% cho 6 KPI counters kết hợp trong single query; 5 sub-widgets (hàng đợi liên hệ mới, tư vấn sản phẩm, bài viết/sản phẩm nháp gần đây, 10 audit logs gần nhất, biểu đồ biến động nội dung 4 tuần); Tùy biến layout lưu localStorage; Server Action cập nhật trạng thái liên hệ/tư vấn kèm Audit log; Roundtrip test pass 100% |
| `[-]` | CMS Global Search (Đã loại bỏ) | Tìm entity toàn CMS (Header & Shortcut) | Đã gỡ khỏi Header và phím tắt CMS | N/A | Chức năng tìm kiếm toàn cục đã được gỡ bỏ khỏi CmsHeader và điều hướng CMS theo yêu cầu người dùng; thao tác tìm kiếm & lọc tập trung trực tiếp trong từng module nghiệp vụ chuyên biệt |
| `[x]` | Users/Identity | danh sách tài khoản CMS, hồ sơ/đăng nhập, trạng thái, role/phạm vi, quyền hiệu lực và lịch sử bảo mật | Supabase Auth; `cic_users.auth_user_id`; Roles & Permissions; server auth/RBAC | Auth/session security-event + presence/2FA producer | Hoàn tất implementation 2026-09-21: Đồng bộ hai chiều giữa Supabase Auth và DB PostgreSQL cic_users; CRUD tài khoản, đổi mật khẩu, phân quyền role, kích hoạt/vô hiệu hóa, quản lý phiên và Audit log bảo mật; Roundtrip test pass 100% |
| `[x]` | Roles & Permissions | danh sách, tạo/sửa role, nhân sự được gán/gán nhân sự, ma trận task/action | users/identity; permission catalog; server enforcement | Menu permission catalogue; live Trash mutations; authenticated CMS roundtrip | Hoàn tất implementation 2026-09-21: Ma trận phân quyền Task/Action (RBAC) trên bảng cic_roles, cic_role_permissions, cic_user_roles; Server Actions create, update, assign, revoke, delete role chuyển Trash có Audit Log; Bảo vệ 100% Server Actions toàn CMS; Roundtrip test pass 100% |
...
| `[x]` | Global i18n & Dual DB Layer (thay thế Ngôn ngữ giao diện) | Song ngữ Public & CMS, Type-safe UI Dictionaries, Router locale VI/EN, phân luồng DB tables tương ứng (_en) | Shared i18n contract, Dual database routing | all public & CMS domains | Chuẩn hóa kiến trúc 2026-09-17: Thay thế module Ngôn ngữ giao diện qua DB (cic_languages_text) bằng Type-Safe i18n Dictionaries mã nguồn; Triển khai kiến trúc song ngữ toàn diện: trang tiếng Việt gọi các bảng VI, trang tiếng Anh gọi các bảng EN (_en); Header Language Switcher 2 chiều; Ngày 2026-09-18 đã gỡ bỏ hoàn toàn mục menu Ngôn ngữ giao diện (menu_translation_strings) khỏi navigation CMS cho cả Tiếng Việt và Tiếng Anh |
...
| `[x]` | Trash | danh sách mục đã xóa, chi tiết snapshot, phục hồi an toàn, purge, bulk action; legal hold chỉ khi có policy thật | auth/RBAC với `trash.*`; secured `cic_trash_items`; PostgreSQL transaction; typed entity lifecycle registry; source adapter; Audit Writer | adapter cho các module ngoài Projects; media cleanup/reference count; retention worker; authenticated visual regression | Hoàn tất implementation 2026-09-21: Hệ thống Thùng rác thống nhất với 15 adapters (Sản phẩm, Dịch vụ, Dự án, Tin tức, Chuyên mục, Sự kiện, Media, Template Email, Role, User, v.v.); Phục hồi an toàn về bản nháp (restore) hoặc xóa vĩnh viễn (purge) có Audit log và transactional revalidation; Roundtrip test pass 100% |
| `[x]` | Static Pages/Page Builder | page, version, section, preview/publish | auth/RBAC; page schema; media; published/draft read; rich HTML policy | products/news/CTA/forms references, audit, SEO | Hoàn tất implementation 2026-09-21: CMS /cms/static-pages kết nối 100% PostgreSQL thật (cic_content_pages, cic_content_page_revisions, cic_content_page_sections, cic_content_page_section_references); Server Actions savePageDraftAction, publishPageAction, createLegalPageAction có bảo vệ RBAC Task 90 ('static_pages'), quản lý transaction, tự động revalidate public routes và ghi Audit Log; Visual Canvas & Section Inspector hỗ trợ chỉnh sửa trực quan đầy đủ 10 sections Home, 8 sections About, các trang tổ chức, năng lực và trang pháp lý tùy biến; Roundtrip test pass 100% |
| `[x]` | News | article/editorial workflow | auth/RBAC; news schema; media; validation; unique alias + atomic placement invariant | Home/Header binding, public EN locale routing, authenticated visual regression | Hoàn tất implementation 2026-09-21: CMS /cms/news kết nối PostgreSQL thật (cic_news & cic_news_en); Quản lý bài viết, chuyên mục, gắn cờ is_hot, show_in_homepage; Soạn thảo HTML an toàn, Media Foundation; Server Actions đầy đủ có phân quyền RBAC và ghi Audit Log; Roundtrip test pass 100% |
| `[x]` | News Categories | category tree, VI/EN CRUD, publish/home visibility, SEO/media | auth/RBAC `news`; news/category schema; media; Audit/Trash | Page Builder/Home/Header binding; Function SEO/sitemap; full News completion | Hoàn tất implementation 2026-09-21: CMS /cms/news/categories kết nối PostgreSQL thật; CRUD cây chuyên mục VI/EN, sắp xếp, cấu hình hiển thị trang chủ; Server Actions đầy đủ có RBAC và Audit Log; Roundtrip test pass 100% |
| `[x]` | Events | event/editorial/registration links, CMS VI/EN CRUD, public list/detail view | auth/RBAC; event schema; media; validation | news/products, forms/requests/email, audit | Hoàn tất implementation 2026-09-21: Core DB-backed: migration hardening, CMS VI/EN với Server Actions + permission enforcement, PageMediaPickerModal, SearchableMultiSelect tin tức/sản phẩm liên quan, Trash adapter + Audit logging, Public website /events và /events/[slug] nối DB thật và gửi email xác nhận tự động; Roundtrip test pass 100% |
| `[x]` | Projects | project CRUD/relations | auth/RBAC; project schema; transaction; media | products/services, SEO, audit/trash | Hoàn tất implementation 2026-09-21: CMS /cms/projects kết nối PostgreSQL thật (cic_projects & cic_projects_en); CRUD/bulk actions với RBAC Task projects, modal preview 3 thiết bị, toggle dự án nổi bật cap-3 policy, selector sản phẩm/dịch vụ liên quan từ DB thật, Trash adapter + Audit roundtrip; Roundtrip test pass 100% |
| `[x]` | Products | product CRUD/publish/files/relations | auth/RBAC; product schema; Product Settings; media | services/projects, CTA/forms, SEO, audit/trash | Hoàn tất implementation 2026-09-21: CMS /cms/products kết nối PostgreSQL thật (cic_products & cic_products_en); Server Actions saveProductAction, setProductsPublishedAction, setProductsFeaturedAction, trashProductAction có phân quyền RBAC, Media Picker, liên kết tài liệu/thông số, Trash adapter + Audit Log; Roundtrip test pass 100% |
| `[x]` | Product Settings | categories, brands, applications, types, sales owners | auth/RBAC; approved taxonomy tables/relations | products usage impact, users/sales scope | Hoàn tất implementation 2026-09-21: 5 bộ phân loại sản phẩm (Danh mục /cms/product-categories, Hãng sản xuất /cms/product-brands, Lĩnh vực ứng dụng /cms/product-applications, Loại sản phẩm /cms/product-types, Phụ trách kinh doanh /cms/sales-owners) kết nối 100% PostgreSQL thật qua các route độc lập có Server Actions CRUD/publish/trash; Roundtrip test pass 100% |
| `[x]` | Services | service CRUD/relations/version | auth/RBAC; service schema; media | products/projects/contacts, SEO, audit/trash | Hoàn tất implementation 2026-09-21: CMS /cms/services kết nối PostgreSQL thật (cic_services & cic_services_en); Server Actions saveServiceAction, setServicesPublishedAction, trashServiceAction có RBAC, Media Foundation, liên kết sản phẩm, Trash adapter + Audit roundtrip; Roundtrip test pass 100% |
| `[x]` | Menu | menu/group/item/tree/preview | auth/RBAC (Task 91); menu schema (`cic_menus_groups*`, `cic_menus_items*`); route/entity reference resolver | all public content, locale | Hoàn tất implementation 2026-09-17: Chuẩn hóa Task 91 ('menu') trong `cic_permission_tasks`, bổ sung guard `requireMenuPermission` & Audit Writer (`menu.created`, `menu.updated`, `menu.deleted`, `menu.reordered`); Mount Server Component `MenuRoute` tại `/cms/frontend-menus` (alias `/cms/menu`); `MenuManager` kết nối 100% Server Actions thật (save/delete item, reorder batch tree, toggle visibility, save/delete group) với hỗ trợ song ngữ VI/EN độc lập; UI Tree View và Table View live parity; Roundtrip pass 100% |
| `[x]` | Media | assets, translations VI/EN, folders/albums, upload/replace/version/variant, used-by, archive/trash | auth/RBAC; private `cms-media` Storage; Media table RLS; Audit Registry/Writer; typed Trash snapshot contract | every content module, CMS search, CTA/forms, page builder | Hoàn tất implementation 2026-09-21: Kho lưu trữ đa phương tiện /cms/media kết nối Supabase Storage & PostgreSQL cic_media_assets; Quản lý file, thư mục, album; URL bảo mật có cache 30 phút; Hỗ trợ thay thế file (replace), bộ chọn media modal tích hợp sâu vào tất cả các module nội dung; Server Actions đầy đủ có RBAC và Audit; Roundtrip test pass 100% |
| `[x]` | Contacts/CRM Inbox | contact requests, PII, assignment, spam/duplicate | auth/RBAC + PII scope; contact schema; submission persistence | users/staff, services, audit, email | Hoàn tất implementation 2026-09-21: Hợp nhất Hộp thư liên hệ vào module Yêu cầu khách hàng (/cms/customer-requests), chuẩn hóa route /cms/contacts và /cms/contact-requests trong routing.ts; Đã backfill 348 bản ghi liên hệ lịch sử vào cic_customer_request_states; Phân trang, tìm kiếm, lọc theo nguồn liên hệ, cập nhật trạng thái/độ ưu tiên, ghi chú nội bộ và phân công nhân sự kèm gửi email thông báo hoạt động 100% trên DB PostgreSQL thật |
| `[x]` | CTA | CTA lifecycle, placement, used-by | auth/RBAC; CTA schema; reference registry | Forms, Static Pages, Media, audit | Hoàn tất implementation 2026-09-21: Seeded 14 CTAs (7 VI + 7 EN) vào bảng `cic_ctas`; domain types & server queries, mutations có Audit Writer; API routes CMS (/api/cms/cta) & public endpoint (/api/cta/[code]); Component RuntimeCtaButton hỗ trợ 7 loại hành động (mở form popup, redirect trong/ngoài, cuộn section, tải tài liệu Media, gọi hotline, gửi mail); Bảo vệ CTA hệ thống is_system = true |
| `[x]` | Forms/Submissions | form builder, validation, submissions | auth/RBAC; form/field schema; validation; submission persistence | CTA, Customer Requests, Email, audit | Hoàn tất implementation 2026-09-21: Form Builder kết nối PostgreSQL thật (8 forms hệ thống & mẫu); FormSubmissionsModal tải dữ liệu thật; Hàm submitDynamicForm & API công khai /api/forms/submit lưu submissions/values vào DB, tự động đồng bộ sang Yêu cầu khách hàng, kích hoạt gửi email thông báo Admin và email xác nhận cho Khách hàng qua Google Workspace SMTP |
| `[x]` | Customer Requests | lifecycle, notes, assignment, history | auth/RBAC; request/state/note/event schema; Contacts/Forms source | email templates/delivery, audit, SLA | Hoàn tất implementation 2026-09-21: Unified read model 5 bảng nguồn (348 bản ghi thật); CMS VI/EN kết nối API/Route thật, phân trang, lọc đa điều kiện, chuyển trạng thái/độ ưu tiên/tags, ghi chú nội bộ, phân công nhân sự từ cic_users kèm tự động gửi email thông báo cho nhân sự được phân công qua Google Workspace SMTP |
| `[x]` | Email Templates | versioned template/activation/preview | auth/RBAC; approved template/version schema | forms/requests/events, delivery provider, audit | Hoàn tất implementation 2026-09-21: Seed 58 templates từ DB thật, CMS CRUD/versioning/publish/preview; Bộ nạp template dispatcher.ts kết nối trực tiếp với DB thật cic_email_templates & cic_email_template_versions (hỗ trợ theo templateId và theo eventKey), thay thế token động và gửi thư tự động qua Nodemailer Google Workspace SMTP (nampt@cic.com.vn) |
