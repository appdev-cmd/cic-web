...
|---|---|---|---|---|---|
| `[C]` | Dashboard | aggregates, notifications, work queues | CMS auth/RBAC; read models từ domain nguồn | analytics, audit, workflow queues | Hoàn tất implementation 2026-09-18: PostgreSQL aggregate read-model thật 100% cho 6 KPI counters, hàng đợi liên hệ mới (cic_contact/cic_contact_en), hàng đợi tư vấn sản phẩm (cic_product_contact), bài viết/sản phẩm nháp gần đây, 10 audit logs gần nhất (cic_activity_logs join cic_users), biểu đồ biến động nội dung 4 tuần gần nhất; Tách module thành 5 sub-widgets độc lập; Tích hợp tùy biến layout lưu localStorage; Server Action cập nhật trạng thái liên hệ/tư vấn kèm Audit log |
